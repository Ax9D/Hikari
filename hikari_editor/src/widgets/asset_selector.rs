...
fn handle_to_path<T: Asset>(handle: &Option<Handle<T>>, asset_manager: &AssetManager) -> Option<PathBuf> {
...
impl<'ui, L: AsRef<str>, const N: usize> AssetSelector<'ui, L, N> {
    pub fn new(ui: &'ui Ui, name: L,
    extensions: [&'static str; N]) -> Self {
        Self {
            ui,
            name,
            extensions
    ...
    pub fn build<T: Asset>(self, return_handle: &mut Option<Handle<T>>, asset_manager: &AssetManager) -> bool {
    ...
    let path_selector = ui.button_with_size(display_path, [ui.content_region_avail()[0] - 20.0, 0.0]);
    ...
    if let Some(_popup_token) = fancy_popup(ui, "popup", window_flags) {
...

        {
            ...
            ui
                .input_text("##AssetPathSearch", search_buffer)
                .hint("Search")
        ...
        for record in filtered_records {
            ...
            if path_str.to_lowercase().contains(&search_buffer.to_lowercase()) {

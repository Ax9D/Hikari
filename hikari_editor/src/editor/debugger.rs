use super::Editor;
use crate::imgui;
use hikari::{pbr::WorldRenderer, asset::{AssetManager, AssetDB}};
use hikari_editor::*;
use imgui::{TableColumnSetup, TableFlags, StorageExt};
        ...
        if let Some(_token) = ui.tab_bar("Render Graph Tabs") {
            ...
            if let Some(_token) = ui.tab_item("Asset DB") {
                ...
                if ui.is_item_hovered() {
                    ui.tooltip_text("Removes all assets which aren't referred to by any other asset");
                }
                ...
                if let Some(size) = ui.storage().get::<String>(imgui::Id::Int(ASSET_FILE_SIZE, ui)) {
                    ui.same_line();
                    ui.text(format!("Estimated Size: {}", size));
                }

                if let Some(_token) = ui.begin_table_header_with_flags("AssetInfo", [
                        TableColumnSetup::new("Path"), 
                        TableColumnSetup::new("UUID")],
                        TableFlags::BORDERS | TableFlags::ROW_BG | TableFlags::RESIZABLE | TableFlags::SCROLL_X | TableFlags::SCROLL_Y
                    ) {
                    ui.table_next_row();

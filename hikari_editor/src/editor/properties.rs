...
pub fn draw(ui: &imgui::Ui, editor: &mut Editor, state: EngineState) -> anyhow::Result<()> {
    let result = ui.window("Properties")
        ...
        if let Some(entity) = outliner.selected {
            ...
            for ty in type_ids {
                if let Some(dispatch) = components.get(ty) {
                    let _token = ui.tree_node_config(dispatch.name())
                    ...
                    ui.same_line_with_pos(ui.window_content_region_max()[0] - ui.window_content_region_min()[0] - ui.frame_height() / 2.0 + 1.0);

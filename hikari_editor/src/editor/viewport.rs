...
impl Default for GizmoState {
    fn default() -> Self {
        Self { context: Default::default(), operation: Operation::Translate, mode: Mode::World }
    }
}
#[derive(Default)]
pub struct Viewport {
...
fn gizmo_toolbar(ui: &imgui::Ui, state: &mut GizmoState, editor_camera: &mut Camera) {
    ...
    .build(|| {
        fn draw_operation(ui: &imgui::Ui, operation: Operation, icon: &str, current: &mut Operation) {
            let mut _style_vars = None;
            if *current == operation {
                let token1 = ui.push_style_color(imgui::StyleColor::Button, imgui::ImColor32::from_rgb(0,115,207).to_rgba_f32s());
                let token2 = ui.push_style_color(imgui::StyleColor::ButtonHovered, imgui::ImColor32::from_rgb(1,151,246).to_rgba_f32s());
        ...
        draw_operation(ui,Operation::Translate,icons::GIZMO_TRANSLATE, &mut state.operation);
        ui.same_line();

        draw_operation(ui,Operation::Rotate, icons::GIZMO_ROTATE, &mut state.operation);
        ui.same_line();

        draw_operation(ui,Operation::Scale, icons::GIZMO_SCALE,&mut state.operation);
        ui.same_line();
...
fn draw_dir_light(ui: &imgui::Ui, world: &mut World, viewport_min: Vec2, viewport_max: Vec2) {
    ...
    if let Some((_, (transform, _))) = world.query::<(&Transform, &Light)>().iter().filter(|(_, (_, light))|{
        ...
        let view_proj = camera.get_projection_matrix(viewport.width(), viewport.height()) * camera_transform.get_matrix().inverse();  
        ...
        if let Some((start, end)) = start_2d.zip(end_2d) {
            ui.get_window_draw_list().add_line(start, end, imgui::ImColor32::WHITE).thickness(1.0).build();
        }
...
fn get_editor_camera(world: &mut World) -> Entity {
    ...
    if create_camera {
        let camera_entity = world.create_entity_with((Transform::default(), EditorOnly, Camera::default()));
        ...
        .build(|| {
            ...
            if let Some(entity) = outliner.selected {
                ...
                if let Ok(mut transform) = world.get_component::<&mut Transform>(entity) {
                    ...
                    if let Some(changed_transform)  = viewport.gizmo_state.context.gizmo(ui)

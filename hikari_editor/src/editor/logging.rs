...
impl Logging {
    pub fn new(log_listener: LogListener) -> Self {
        Self { log_listener, filter: 0, search: String::new() }
    }
}
...
fn draw_line(ui: &imgui::Ui, line: &Line) {
    ...
    ui.text(line.timestamp.to_rfc3339_opts(chrono::SecondsFormat::Secs, false));
    ui.table_next_column();
    ...
    fn draw(ui: &imgui::Ui, editor: &mut Editor, _state: EngineState) -> anyhow::Result<()> {
        ...
        ui.window("Engine Log")
            ...
            .build(|| {
                ...
                ui.combo("Filter", &mut logging.filter, &[
                    None,
                ...
                ], |level| std::borrow::Cow::Borrowed(level.map(|level|level.as_str()).unwrap_or("All")));
                }
                ...
                if logging.filter == 0 && logging.search.is_empty() {
                    ...
                    .filter(|line| line.message.to_lowercase().contains(&logging.search.to_lowercase()));

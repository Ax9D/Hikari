...
use hikari_render::{
    ...
    vk::{self},
    vk_sync::ImageLayout,
    AccessType, ComputePass, Device, GpuBuffer, GraphBuilder, ImageConfig, SampledImage,
};
...
impl Primitives {
    pub fn prepare(gfx: &mut Gfx, shader_lib: &mut ShaderLibrary) -> Arc<Self> {
        ...
        let (checkerboard, width, height) = image::open_rgba8(
            hikari_utils::engine_dir().join("data/assets/textures/checkerboard.png"),
        )
        ...
        let black_cube = TextureCube::new(
            device,
            &[0_u8, 0, 0, 1].repeat(6),
            1,
            1,
...
fn load_cube(device: &Arc<Device>) -> anyhow::Result<Cube> {
    let cube =
        crate::old::Scene::load(hikari_utils::engine_dir().join("data/assets/models/cube.glb"))?;

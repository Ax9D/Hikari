...
use crate::{Args, world::WorldUBO, light::CascadeRenderInfo, Settings};
use hikari_3d::*;
use hikari_math::*;
use hikari_render::*;
...
pub fn compute_cascades(world_ubo: &mut WorldUBO, settings: &Settings) {
    ...
    for i in 0..N_CASCADES {
            ...
            let atlas_size_ratio = Vec2::new(shadow_map_size as f32 / atlas_width as f32, shadow_map_size as f32 / atlas_height as f32);
...
pub fn create_hi_z_images(device: &Arc<Device>, mut width: u32, mut height: u32) -> anyhow::Result<Vec<SampledImage>> {
...
depth_prepass: &GpuHandle<SampledImage>
) -> anyhow::Result<(GpuHandle<SampledImage>, GpuHandle<GpuBuffer<CascadeRenderInfo>>)>{
    let shadow_map_size = settings.directional_shadow_map_resolution.size();
    ...
    graph.add_computepass(
        ComputePass::<Args>::new("HierarchicalDepthGeneration")
            .read_image(&depth_prepass, AccessType::ComputeShaderReadSampledImageOrUniformTexelBuffer)
            .cmd(move |cmd, graph_res, _record_info, (_world, res, shader_lib, _assets)| {
                ...
                for i in 1..hiz_images.len() {
                    ...
                    cmd.apply_image_barrier(&hiz_images[i - 1], 
    ...
    let cascade_render_buffer = GpuBuffer::<CascadeRenderInfo>::new(device, N_CASCADES, vk::BufferUsageFlags::STORAGE_BUFFER)?;
    ...
    graph.add_computepass(ComputePass::<Args>::new("ShadowCascadeGeneration")
    ...
    .cmd(move |cmd, graph_res, _, (world, res, shader_lib, _assets)| {
        if let Some(dir_light) =res.directional_light {
            ...
            cmd.apply_image_barrier(&reduced_depth_image, 
                &[AccessType::ComputeShaderWrite],
                 &[AccessType::ComputeShaderReadOther], 
                 vk_sync::ImageLayout::General, 
                 vk_sync::ImageLayout::General,
    ...
    graph.add_renderpass(Renderpass::<Args>::new("ShadowMapping", atlas_size)
    ...
    .cmd( move |cmd, graph_res, _, (world, res, shader_lib, assets)| {
        ...
        if let Some(dir_light) = dir_light {
            ...
            cmd.set_rasterizer_state(RasterizerState {
                cull_mode: if shadow_info.cull_front_face { CullMode::Front } else { CullMode::Back },

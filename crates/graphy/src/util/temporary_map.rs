...
pub struct TemporaryMap<K, V, const N: usize> {
    hashmap: FxHashMap<KeyRef<K>, *mut Node<Entry<K, V>>>,
...
impl<K: Hash + Eq, V, const N: usize> TemporaryMap<K, V, N> {
    pub fn new() -> Self {
        ...
        let frames = frames.into_inner().map_err(|_| "Array Vec to array conversion failed").unwrap();
    ...
    pub fn insert<'a>(&'a mut self, key: K, value: V) -> Option<V>{
        ...
        match self.hashmap.get(&key_ref) {
            ...
            None => {
                ...
                self.hashmap.insert(KeyRef {key: &unsafe { &mut *new_node }.data_mut().key}, new_node);

...
layout(location = 7) uniform int albedoUVSet;
layout(location = 8) uniform int roughnessUVSet;
layout(location = 9) uniform int metallicUVSet;

layout(location = 10) uniform int normalUVSet;
...
layout(location = 20) uniform float exposure;
...
void main() {
    ...
    vec3 albedoValue = getValueRGBA(albedoMap, albedo, albedoUVSet).rgb;
    float roughnessValue = getValueFloat(roughnessMap, roughness, roughnessUVSet).g;
    //roughnessValue/=roughness + 0.001;

    float metallicValue = getValueFloat(metallicMap, metallic, metallicUVSet).b;
    //metallicValue/=metallic + 0.001;

    vec3 normal = normalUVSet > -1 ? calculateNormal() : normalize(normalFs);
